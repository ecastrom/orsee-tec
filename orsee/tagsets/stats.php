...
function stats__get_data($condition=array(),$type='stats',$restrict=array(),$options=array()) {
    ...
    if ($type=='stats') {
        foreach (array('number_reg','number_noshowup','last_enrolment',
...
function stats__report_display_table($table1,$table1_name,$table2=array(),$table2_name="",$table3=array(),$table3_name="") {
    ...
    // order keys
    $allvals=array();
    foreach ($table1['data'] as $k=>$v) {
        $allvals[$k]=$v;
    }
    if ($has_data2) {
        foreach ($table2['data'] as $k=>$v) {
            if (!isset($allvals[$k]) || $v>$allvals[$k]) {
                $allvals[$k]=$v;
            }
        }
    }
    if ($has_data3) {
        foreach ($table3['data'] as $k=>$v) {
            if (!isset($allvals[$k]) || $v>$allvals[$k]) {
                $allvals[$k]=$v;
            }
        }
    ...
    foreach ($allvals as $k=>$v) {
        if ($i<$settings['stats_report_rows_limit']) {
            $usevals[]=$k;
        }
        $i++;
    }
    ...
    if ($has_table2) {
        $out.= '<div class="orsee-table-cell" style="border-bottom: 0;"></div>';
    }
    if ($has_table3) {
        $out.= '<div class="orsee-table-cell" style="border-bottom: 0;"></div>';
    }
    ...
    if ($has_table2) {
        $out.= '<div class="orsee-table-cell" style="border-bottom: 0;">'.$table2_name.'</div>';
    }
    if ($has_table3) {
        $out.= '<div class="orsee-table-cell" style="border-bottom: 0;">'.$table3_name.'</div>';
    }
    $out.= '</div>';
    $out.= '<div class="orsee-table-row orsee-table-head">
    ...
    if ($has_table2) {
        $out.= '<div class="orsee-table-cell">N='.$table2['N'].'</div>';
    }
    if ($has_table3) {
        $out.= '<div class="orsee-table-cell">N='.$table3['N'].'</div>';
    }
    $out.= '</div>';
    $shade=false;
    $other=array('t1'=>0,'t2'=>0,'t3'=>0);
    foreach ($table1['data'] as $k=>$value) {
        if (in_array($k,$usevals)) {
            ...
            if (isset($table1['value_names'][$k])) {
                $out.= $table1['value_names'][$k];
            } else {
                $out.=$k;
            }
            $out.= '</div>';
            ...
            if ($table1['type_of_data']=='count') {
                $out.= round(($value/$table1['N'])*100,1).'%';
            } else {
                $out.= $value;
            }
            $out.='</div>';
            if ($has_table2) {
                ...
                if ($has_data2 && $table2['N']>0) {
                    if (isset($table2['data'][$k])) {
                        if ($table1['type_of_data']=='count') {
                            $out.= round(($table2['data'][$k]/$table2['N'])*100,1).'%';
                        } else {
                            $out.= $table2['data'][$k];
                        }
                    } else {
            ...
            if ($has_table3) {
                ...
                if ($has_data3 && $table3['N']>0) {
                    if (isset($table3['data'][$k])) {
                        if ($table1['type_of_data']=='count') {
                            $out.= round(($table3['data'][$k]/$table3['N'])*100,1).'%';
                        } else {
                            $out.= $table3['data'][$k];
                        }
                    } else {
            ...
            $other['t1']+=$value;
            if (isset($table2['data'][$k])) {
                $other['t2']+=$table2['data'][$k];
            }
            if (isset($table3['data'][$k])) {
                $other['t3']+=$table3['data'][$k];
            }
        }
    }
    if ($other['t1']>0 || $other['t2']>0 || $other['t3']>0) {
        ...
        $out.= '<div class="orsee-table-cell">';
        if ($table1['type_of_data']=='count') {
            $out.= round(($other['t1']/$table1['N'])*100,1).'%';
        } else {
            $out.= $other['t1'];
        }
        $out.='</div>';
        if ($has_table2) {
            ...
            if ($has_data2) {
                if ($table1['type_of_data']=='count') {
                    $out.= round(($other['t2']/$table2['N'])*100,1).'%';
                } else {
                    $out.= $other['t2'];
                }
            } else {
        ...
        if ($has_table3) {
            ...
            if ($has_data3) {
                if ($table1['type_of_data']=='count') {
                    $out.= round(($other['t3']/$table3['N'])*100,1).'%';
                } else {
                    $out.= $other['t3'];
                }
            } else {
...
function stats__stats_get_table_array($table) {
    ...
    if ($multi) {
        ...
        $ta['multi_header'][]='';
        foreach ($table['column_names'] as $k=>$col) {
            $ta['multi_header'][]=$col;
        }
    }
    $ta['header']=array();
    ...
    foreach ($allvals as $k) {
        $row=array();
        if (isset($table['value_names'][$k])) {
            $row[]= $table['value_names'][$k];
        } else {
            $row[]=$k;
        }
        if ($multi) {
            foreach ($table['column_names'] as $c=>$col) {
                if (isset($table['data'][$c][$k])) {
                    $row[]= $table['data'][$c][$k];
                } else {
                    $row[]= '0';
                }
...
function stats__stats_display_table($table,$browsable=false,$restrict=array()) {
    if ($browsable && isset($table['browsable']) && $table['browsable']) {
        $browsable=true;
    } else {
        $browsable=false;
    }
    ...
    if (isset($ta['multi_header'])) {
        ...
        if ($browsable) {
            $out.='<TD class="orsee-table-cell"></TD>';
        }
        foreach ($ta['multi_header'] as $c) {
            $out.='<TD class="orsee-table-cell">'.$c.'</TD>';
        }
        $out.= '</TR>';
    }
    if (isset($ta['header'])) {
        $out.= '<TR class="orsee-table-row orsee-table-head">';
        if ($browsable) {
            $out.='<TD class="orsee-table-cell">'.lang('filter_out').'</TD>';
        }
        foreach ($ta['header'] as $c) {
            $out.='<TD class="orsee-table-cell">'.$c.'</TD>';
        }
        $out.= '</TR>';
    }
    ...
    if (isset($table['name']) && isset($restrict[$table['name']]) && $browsable) {
        foreach ($restrict[$table['name']] as $k=>$v) {
            ...
            if (isset($table['value_names'][$k])) {
                $out.='<TD class="orsee-table-cell" '.$style_hl.'>'.$table['value_names'][$k].'</TD>';
            } else {
                $out.='<TD class="orsee-table-cell" '.$style_hl.'>'.$k.'</TD>';
            }
            if ($table['charttype']=='multibars') {
                foreach ($table['column_names'] as $c=>$col) {
                    $out.='<TD class="orsee-table-cell" '.$style_hl.'>-</TD>';
                }
            } else {
                $out.='<TD class="orsee-table-cell" '.$style_hl.'>-</TD>';
            }
...
function stats__stats_display_textstats($table) {
    ...
    $len=array();
    if (isset($ta['multiheader'])) {
        $len=stats__textstats_max_col_width($ta['multiheader'],$len);
    }
    if (isset($ta['header'])) {
        $len=stats__textstats_max_col_width($ta['header'],$len);
    }
    foreach ($ta['data'] as $row) {
        $len=stats__textstats_max_col_width($row,$len);
    }

    $out="";
    $out.="**".$table['title']."\n";
    if (isset($ta['multiheader'])) {
        $out.=stats__textstats_display_row($ta['multiheader'],$len);
    }
    if (isset($ta['header'])) {
        $out.=stats__textstats_display_row($ta['header'],$len);
    }
    foreach ($ta['data'] as $row) {
        $out.=stats__textstats_display_row($row,$len);
    }
    return $out;
}
...
function stats__get_y_increment($data) {
    ...
    if ($big_scaled<=5) {
        $inc=0.5 * pow(10,$dec_places);
    } else {
        $inc=1 * pow(10,$dec_places);
    }
    return $inc;
}
