...
function load_language($language) {
    ...
    if (in_array($language,$languages)) {
        ...
        $this_lang=$settings['public_standard_language'];
    }
...
function language__get_item($content_type,$content_name,$language="") {
    ...
    if (isset($line[$language])) {
        return stripslashes($line[$language]);
    } else {
        return false;
    }
}
...
function lang__get_lang_id($content_type,$content_name) {
    ...
    if (is_array($line) && isset($line['lang_id']) && (int)$line['lang_id']>0) {
        return (int)$line['lang_id'];
    }
    return 0;
}
...
function lang__is_rtl_all_langs() {
    ...
    if (isset($preloaded_lang_is_rtl) && is_array($preloaded_lang_is_rtl)) {
...
function lang__select_lang($varname,$selected="",$type="all",$select_wrapper_class='select is-primary',$compact=false,$select_class='') {
    global $lang;
    switch ($type) {
        case "public": $sel_langs=lang__get_public_langs();
            break;
        case "part": $sel_langs=lang__get_part_langs();
            break;
...
function lang__compact_language_switch($current_language='',$type='public',$select_id='orsee-language-switch-compact',$param_name='language') {
    ...
    switch ($type) {
        case 'part': $sel_langs=lang__get_part_langs();
            break;
        case 'all': $sel_langs=get_languages();
            break;
        default: $sel_langs=lang__get_public_langs();
    }
    if (!in_array($current_language,$sel_langs,true) && isset($sel_langs[0])) {
        $current_language=$sel_langs[0];
    }
...
function lang__load_lang_cat($content_type,$language="",$order='content_name') {
    ...
    if ($order==='fixed_order') {
        $order_by='order_number';
    } elseif ($order==='alphabetically') {
        $order_by=$language;
    } else {
        $order_by='content_name';
    }
